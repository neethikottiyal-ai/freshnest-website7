...
const services = [
  ...
  { id: "mattress-pest", cat: "Pest Control", name: "Mattress Pest Control", price: 999, unit: "unit", qty: 1, icon: BedDouble, image: "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=900&q=80", tag: "Bed Bugs", duration: "1-2 hrs" },
];
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="text-2xl font-black tracking-tight">
          <span className={dark ? "text-white" : "text-[#061b3f]"}>Fresh</span><span className="text-[#f5b72f]">Nest</span>
        </div>
        <div className={dark ? "text-[10px] font-black tracking-[0.28em] text-white/70" : "text-[10px] font-black tracking-[0.28em] text-slate-500"}>CLEANING SERVICES</div>
      </div>
    </div>
  );
}
...
export default function FreshNestCustomerWebsite() {
  ...
  const [callback, setCallback] = useState({ name: "", phone: "", time: "Call me in 5 mins" });

  const filtered = useMemo(() => category === "All" ? services : services.filter((service) => service.cat === category), [category]);
  ...
  const canConfirm = cart.length > 0 && booking.name.trim() && booking.phone.length >= 10 && booking.address.trim() && booking.date && slot && payment && !saving;
  ...
  function addService(service) {
    setCart((old) => {
      ...
      if (found) return old.map((item) => item.id === service.id ? { ...item, qty: Number(item.qty || 0) + Number(service.qty || 1) } : item);
      return [...old, { ...service, qty: Number(service.qty || 1) }];
    });
  }

  function changeQty(id, delta) {
    setCart((old) => old.map((item) => item.id === id ? { ...item, qty: Math.max(0, Number(item.qty || 0) + Number(delta || 0)) } : item).filter((item) => item.qty > 0));
  }
 ...
 function openBooking(service = null) {
  ...
  function useCurrentLocation() {
    if (!navigator.geolocation) {
      alert("Location not supported. Please type address.");
      return;
    }
  ...
  function openGoogleMap() {
    ...
    openSafeUrl(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(query)}`);
  }

  function buildWhatsApp(order) {
    const lines = order.items.map((item) => `- ${item.name}: ${item.qty} ${item.unit} x ${money(item.price)} = ${money(item.total)}`).join("\n");
  ...
  function sendWhatsApp(order) {
    if (!order) return;
    openSafeUrl(`https://wa.me/${COMPANY_PHONE}?text=${buildWhatsApp(order)}`);
  }
  ...
  async function confirmBooking() {
    ...
    const order = {
      ...
      mapLocation: booking.mapLocation.trim(),
      date: booking.date,
      slot,
      payment,
      ...
      total,
      source: "Website",
      paymentStatus: payment === "Cash" ? "Cash on Service" : "Pending",
      status: "Confirmed",
    ...
    try {
      ...
      sendWhatsApp(order);
      alert("Booking saved to Firestore and WhatsApp opened.");
      setCart([]);
      setSlot("");
      setPayment("");
      setGift(false);
      ...
      alert(`Firebase save failed: ${error?.message || "Unknown error"}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  function repeatLast() {
    ...
    setCart(latest.items.map((item) => ({
      ...item,
      image: services.find((service) => service.id === item.id)?.image || "",
      icon: services.find((service) => service.id === item.id)?.icon || Home,
      cat: services.find((service) => service.id === item.id)?.cat || "Service",
      tag: services.find((service) => service.id === item.id)?.tag || "Repeat",
      duration: services.find((service) => service.id === item.id)?.duration || "",
    })));
    setBooking((old) => ({ ...old, name: latest.name, phone: latest.phone, address: latest.address, mapLocation: latest.mapLocation || "" }));
    setPage("booking");
  }

  if (page === "booking") {
    return <BookingPage {...{ cart, addService, changeQty, setItemQty, favorites, toggleFavorite, coupon, setCoupon, subtotal, discount, platformFee, total, booking, setBooking, slot, setSlot, payment, setPayment, canConfirm, confirmBooking, useCurrentLocation, openGoogleMap, latest, repeatLast, setPage, gift, setGift, photoName, setPhotoName, saving }} />;
  }
...
function BookingPage(props) {
  const { cart, addService, changeQty, setItemQty, favorites, toggleFavorite, coupon, setCoupon, subtotal, discount, platformFee, total, booking, setBooking, slot, setSlot, payment, setPayment, canConfirm, confirmBooking, useCurrentLocation, openGoogleMap, latest, repeatLast, setPage, gift, setGift, photoName, setPhotoName, saving } = props;
  return (
          ...
          return (
            <div key={service.id} className="rounded-xl border border-slate-100 p-3">
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => toggleFavorite(service.id)} className={favorites.includes(service.id) ? "text-red-500" : "text-slate-300"}>
                  <Heart fill={favorites.includes(service.id) ? "currentColor" : "none"} />
                </button>
              </div>
              {item ? (
                <div className="mt-3 grid grid-cols-[auto_1fr_auto] gap-2">
                  <button type="button" onClick={() => changeQty(service.id, -1)} className="rounded-xl bg-yellow-300 p-2"><Minus size={14} /></button>
                  <input value={item.qty} onChange={(event) => setItemQty(service.id, event.target.value)} className="rounded-xl bg-slate-50 px-3 text-center font-black outline-none" inputMode="decimal" />
                  <button type="button" onClick={() => changeQty(service.id, 1)} className="rounded-xl bg-yellow-300 p-2"><Plus size={14} /></button>
                </div>
              ) : (
                <button type="button" onClick={() => addService(service)} className="mt-3 rounded-lg bg-yellow-400 px-4 py-2 text-sm font-black">+ Add</button>
              )}
            </div>
          );
        })}
      ...
      <div className="mt-6 rounded-2xl bg-slate-50 p-4">
        ...
        {cart.length === 0 ? <p className="text-sm font-bold text-slate-500">No services added.</p> : cart.map((item) => (
          <div key={item.id} className="mb-2 flex items-center gap-3 rounded-xl bg-white p-3">
            <img src={item.image || "https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=900&q=80"} className="h-12 w-12 rounded-lg object-cover" alt={item.name} />
            ...
            <button type="button" onClick={() => changeQty(item.id, -item.qty)} className="rounded-lg bg-red-50 p-2 text-red-600"><Trash2 size={15} /></button>
          </div>
        ))}
        <div className="mt-4 grid gap-2 md:grid-cols-3">{coupons.map((item) => <button type="button" key={item.code} onClick={() => setCoupon(item.code)} className={`rounded-xl border p-3 text-left ${coupon === item.code ? "border-yellow-400 bg-yellow-50" : "border-slate-100 bg-white"}`}><p className="font-black">{item.code}</p><p className="text-xs text-slate-500">Off {money(item.value)}</p></button>)}</div>
...
function AddressPaymentConfirm({ booking, setBooking, slot, setSlot, payment, setPayment, useCurrentLocation, openGoogleMap, canConfirm, confirmBooking, total, gift, setGift, photoName, setPhotoName, saving }) {
  return (
    <Card>
      <SectionTitle n="2" title="Address, Payment & Confirm" sub="Fill all fields to enable Firestore save and WhatsApp booking." />
      ...
      <div className="rounded-[1.5rem] border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-4">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <input value={booking.phone} onChange={(event) => setBooking({ ...booking, phone: event.target.value.replace(/[^0-9]/g, "") })} className="rounded-xl bg-white px-4 py-4 font-bold outline-none" placeholder="Mobile number" inputMode="numeric" />
        </div>
        <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
          <input value={booking.address} onChange={(event) => setBooking({ ...booking, address: event.target.value })} className="rounded-xl bg-white px-4 py-4 font-bold outline-none" placeholder="Full address" />
          <div className="grid gap-2">
            <button type="button" onClick={useCurrentLocation} className="rounded-xl bg-emerald-600 px-5 py-3 font-black text-white"><MapPin className="mr-1 inline" size={16} /> Use Location</button>
            <button type="button" onClick={openGoogleMap} className="rounded-xl bg-blue-600 px-5 py-3 font-black text-white"><MapPin className="mr-1 inline" size={16} /> Open Maps</button>
          </div>
        </div>
        <input value={booking.mapLocation} onChange={(event) => setBooking({ ...booking, mapLocation: event.target.value })} className="mt-3 w-full rounded-xl bg-white px-4 py-4 font-bold outline-none" placeholder="Google Map location URL or lat,long" />
      ...
      <div className="mt-5"><p className="mb-2 font-black">Time Slot</p><div className="grid grid-cols-2 gap-2 md:grid-cols-3">{slots.map((item) => <button type="button" key={item} onClick={() => setSlot(item)} className={`rounded-xl px-3 py-3 text-xs font-black ${slot === item ? "bg-yellow-100 ring-2 ring-yellow-400" : "bg-slate-50"}`}>{item}</button>)}</div></div>
      <div className="mt-5"><p className="mb-2 font-black">Payment</p>{["UPI", "Card", "Cash"].map((item) => <button type="button" key={item} onClick={() => setPayment(item)} className={`mb-3 w-full rounded-xl border p-4 text-left font-black ${payment === item ? "border-yellow-400 bg-yellow-50" : "border-slate-100"}`}>{item}</button>)}</div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm font-black">Total Amount</p><p className="text-3xl font-black">{money(total)}</p></div><button type="button" onClick={confirmBooking} disabled={!canConfirm} className="rounded-xl bg-yellow-400 p-4 font-black disabled:bg-slate-300 disabled:text-slate-500">{saving ? "Saving..." : "Confirm Booking & Send WhatsApp"} <MessageCircle className="ml-2 inline" size={16} /></button></div>
...
function StickySummary({ cart, total, latest, repeatLast }) {
  return <div className="sticky top-24 h-fit rounded-[1.7rem] bg-[#111827] p-5 text-white shadow-2xl"><h3 className="text-xl font-black text-yellow-300">Booking Summary</h3><p className="mt-1 text-sm font-bold text-white/50">{cart.length} services selected</p><div className="mt-4 space-y-3">{cart.length === 0 ? <p className="rounded-xl bg-white/10 p-3 text-sm font-bold text-white/60">No service added.</p> : cart.map((item) => <div key={item.id} className="rounded-xl bg-white/10 p-3"><p className="font-black">{item.name}</p><p className="text-xs text-white/50">{item.qty} {item.unit}</p><p className="font-black text-yellow-300">{money(item.price * item.qty)}</p></div>)}</div><div className="mt-5 border-t border-white/10 pt-4"><p className="text-xs text-white/40">Total</p><p className="text-4xl font-black text-yellow-300">{money(total)}</p></div>{latest && <button type="button" onClick={repeatLast} className="mt-4 w-full rounded-xl bg-yellow-400 p-3 font-black text-black">Repeat Last Booking</button>}</div>;
}

function ServicesSection({ filtered, category, setCategory, favorites, toggleFavorite, openBooking }) {
  return <section id="services" className="py-10"><div className="mb-6"><h2 className="text-3xl font-black">Services</h2><p className="text-sm font-bold text-slate-500">Select service and book. Sq.ft services calculate automatically.</p></div><div className="mb-6 flex flex-wrap gap-4">{categories.map((item) => <button type="button" onClick={() => setCategory(item)} key={item} className={`rounded-xl px-6 py-3 font-black ${category === item ? "bg-yellow-400" : "bg-white"}`}>{item}</button>)}</div><div className="grid gap-6 md:grid-cols-3">{filtered.map((service) => <ServiceCard key={service.id} service={service} openBooking={openBooking} favorite={favorites.includes(service.id)} toggleFavorite={toggleFavorite} />)}</div></section>;
}

function ServiceCard({ service, openBooking, favorite, toggleFavorite }) {
  const Icon = service.icon;
  return <div className="overflow-hidden rounded-[1.5rem] bg-white shadow-sm"><div className="relative h-44"><img src={service.image} alt={service.name} className="h-full w-full object-cover" /><span className="absolute left-4 top-4 rounded-full bg-yellow-300 px-3 py-1 text-xs font-black">{service.tag}</span><button type="button" onClick={() => toggleFavorite(service.id)} className={`absolute right-4 top-4 rounded-full bg-white/90 p-2 ${favorite ? "text-red-500" : "text-slate-400"}`}><Heart fill={favorite ? "currentColor" : "none"} /></button></div><div className="p-5"><Icon className="mb-3 text-yellow-600" size={20} /><h3 className="text-xl font-black">{service.name}</h3><p className="mt-2 text-sm font-semibold text-slate-500">{service.duration}</p><div className="mt-4 flex items-end justify-between"><div><p className="text-2xl font-black">{money(service.price)} <span className="text-sm text-slate-500">/ {service.unit}</span></p><p className="mt-1 text-xs font-bold text-slate-500"><Star className="mr-1 inline text-yellow-400" size={13} />4.8</p></div><button type="button" onClick={() => openBooking(service)} className="rounded-xl bg-yellow-300 px-5 py-3 font-black">Book</button></div></div></div>;
}
...
function StickyBookingBar({ openBooking }) {
  return <div className="sticky top-[73px] z-30 border-b border-[#f5b72f]/20 bg-[#061b3f]/95 px-4 py-3 text-white shadow-lg backdrop-blur">
    <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3 text-sm font-black"><span className="rounded-full bg-red-500 px-3 py-1 text-white">Launch Offer</span><span>Flat 15% OFF this week • Premium slots available today</span></div>
      <div className="flex gap-2"><button type="button" onClick={() => openBooking()} className="rounded-xl bg-[#f5b72f] px-4 py-2 font-black text-[#061b3f]">Book Now</button><button type="button" onClick={() => openSafeUrl(`https://wa.me/${COMPANY_PHONE}`)} className="rounded-xl bg-emerald-600 px-4 py-2 font-black text-white">WhatsApp</button></div>
    </div>
  </div>;
}
...
function SavedAddressBox({ setBooking }) {
  ...
  return <Card className="mb-5 bg-gradient-to-br from-yellow-50 to-white"><h3 className="text-xl font-black text-[#061b3f]">Saved Addresses</h3><p className="mt-1 text-sm font-bold text-slate-500">Choose address quickly for repeat booking.</p><div className="mt-4 grid gap-3 md:grid-cols-3">{addresses.map((address) => <button type="button" key={address} onClick={() => setBooking((old) => ({ ...old, address }))} className="rounded-xl border border-yellow-100 bg-white p-3 text-left text-sm font-black shadow-sm">{address}</button>)}</div></Card>;
}

function LaunchOfferPopup({ close, openBooking }) {
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-black/40 p-5 backdrop-blur-sm"><div className="max-w-md rounded-[2rem] bg-white p-8 text-center shadow-2xl"><div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-yellow-100 text-4xl">🎉</div><h2 className="mt-4 text-3xl font-black text-[#061b3f]">Grand Launch Offer</h2><p className="mt-2 font-bold text-slate-600">Book this week and get instant launch discount on cleaning services.</p><div className="mt-5 rounded-2xl bg-[#061b3f] p-5 text-white"><p className="text-sm font-black text-[#f5b72f]">USE COUPON</p><p className="text-3xl font-black">FNFIRST</p></div><button type="button" onClick={() => { close(); openBooking(); }} className="mt-5 w-full rounded-xl bg-[#f5b72f] p-4 font-black text-[#061b3f]">Book Now</button><button type="button" onClick={close} className="mt-3 text-sm font-black text-slate-500">Maybe later</button></div></div>;
}

function RecentBookingToast({ recentIndex, setRecentIndex }) {
  const data = useMemo(() => ["Arun from Trichy booked Sofa Shampooing", "Priya from Srirangam booked Deep Cleaning", "Vignesh from Woraiyur booked Water Tank Cleaning"], []);
  useEffect(() => {
    const timer = setInterval(() => setRecentIndex((old) => (old + 1) % data.length), 5000);
    return () => clearInterval(timer);
  }, [data.length, setRecentIndex]);
...
function OtpLoginModal({ close, otpMobile, setOtpMobile, otpCode, setOtpCode, setLoggedIn }) {
  ...
  return <div className="fixed inset-0 z-[90] grid place-items-center bg-black/40 p-5 backdrop-blur-sm"><div className="w-full max-w-md rounded-[2rem] bg-white p-8 shadow-2xl"><h2 className="text-3xl font-black text-[#061b3f]">Customer Login</h2><p className="mt-2 font-bold text-slate-500">Login with mobile OTP to view booking history and saved address.</p><input value={otpMobile} onChange={(e) => setOtpMobile(e.target.value.replace(/[^0-9]/g, ""))} className="mt-5 w-full rounded-xl bg-slate-50 px-4 py-4 font-bold outline-none" placeholder="Mobile number" /><div className="mt-3 flex gap-2"><input value={otpCode} onChange={(e) => setOtpCode(e.target.value.replace(/[^0-9]/g, ""))} className="flex-1 rounded-xl bg-slate-50 px-4 py-4 font-bold outline-none" placeholder="Enter OTP" /><button type="button" onClick={() => setOtpCode("1234")} className="rounded-xl bg-yellow-400 px-4 py-3 font-black">Send OTP</button></div><button type="button" onClick={verify} className="mt-5 w-full rounded-xl bg-[#061b3f] p-4 font-black text-white">Verify & Login</button><button type="button" onClick={close} className="mt-3 w-full text-sm font-black text-slate-500">Close</button></div></div>;
}
...
function PhaseTwoPowerSections({ latest, bookings, sendWhatsApp, callback, setCallback }) {
  ...
  return <section className="py-12"><div className="mb-7"><h2 className="text-3xl font-black text-[#061b3f]">Customer Power Tools</h2><p className="text-sm font-bold text-slate-500">Booking management, invoice, support and payment.</p></div><div className="grid gap-6 lg:grid-cols-2"><Card className="bg-gradient-to-br from-[#061b3f] to-[#0a2a5c] text-white"><h3 className="text-2xl font-black text-[#f5b72f]">Live Booking Tracker</h3><div className="mt-5 grid gap-3">{tracker.map((step, index) => <div key={step} className="flex items-center gap-3 rounded-xl bg-white/10 p-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-[#f5b72f] font-black text-[#061b3f]">{index + 1}</div><b>{step}</b></div>)}</div></Card><Card><h3 className="text-2xl font-black text-[#061b3f]">Invoice & Payment</h3><p className="mt-2 font-semibold text-slate-500">Share invoice on WhatsApp and prepare online payment.</p><div className="mt-5 grid gap-3 md:grid-cols-2"><button type="button" onClick={() => latest ? sendWhatsApp(latest) : alert("No booking yet")} className="rounded-xl bg-emerald-600 p-4 font-black text-white">WhatsApp Invoice</button><button type="button" onClick={() => alert("PDF invoice module ready placeholder")} className="rounded-xl bg-[#061b3f] p-4 font-black text-white">Download Invoice</button><button type="button" onClick={() => alert("Razorpay/PhonePe gateway can be connected here")} className="rounded-xl bg-yellow-400 p-4 font-black text-[#061b3f] md:col-span-2">Pay Online</button></div></Card><Card><h3 className="text-2xl font-black text-[#061b3f]">Callback Request</h3><div className="mt-4 grid gap-3"><input value={callback.name} onChange={(e) => setCallback({ ...callback, name: e.target.value })} className="rounded-xl bg-slate-50 p-4 font-bold outline-none" placeholder="Name" /><input value={callback.phone} onChange={(e) => setCallback({ ...callback, phone: e.target.value.replace(/[^0-9]/g, "") })} className="rounded-xl bg-slate-50 p-4 font-bold outline-none" placeholder="Phone" /><select value={callback.time} onChange={(e) => setCallback({ ...callback, time: e.target.value })} className="rounded-xl bg-slate-50 p-4 font-bold outline-none"><option>Call me in 5 mins</option><option>Call me today evening</option><option>Call me tomorrow</option></select><button type="button" onClick={() => openSafeUrl(`https://wa.me/${COMPANY_PHONE}?text=${encodeURIComponent(`Callback request: ${callback.name} ${callback.phone} ${callback.time}`)}`)} className="rounded-xl bg-[#061b3f] p-4 font-black text-white">Request Callback</button></div></Card><Card className="bg-gradient-to-br from-yellow-50 to-white"><h3 className="text-2xl font-black text-[#061b3f]">Revisit / Complaint Support</h3><p className="mt-2 font-semibold text-slate-600">Not satisfied? Request a revisit or service support through WhatsApp.</p><button type="button" onClick={() => openSafeUrl(`https://wa.me/${COMPANY_PHONE}?text=${encodeURIComponent("Hi FreshNest, I need revisit/support for my booking")}`)} className="mt-5 rounded-xl bg-yellow-400 px-5 py-4 font-black text-[#061b3f]">Request Support</button><div className="mt-5 rounded-xl bg-white p-4 font-bold text-slate-600">Customer Dashboard: {bookings.length} booking(s) saved</div></Card></div></section>;
}
...
function HomeSections() {
  ...
  return <><section id="how" className="py-10"><h2 className="mb-6 text-3xl font-black">How It Works</h2><div className="grid gap-5 md:grid-cols-4">{[["1", "Choose Service"], ["2", "Fill Details"], ["3", "Firestore Save"], ["4", "WhatsApp Confirm"]].map(([n, t]) => <Card key={n}><div className="grid h-10 w-10 place-items-center rounded-full bg-yellow-400 font-black">{n}</div><h3 className="mt-3 font-black">{t}</h3></Card>)}</div></section><section className="py-10"><h2 className="mb-6 text-3xl font-black">Before / After Work</h2><div className="grid gap-5 md:grid-cols-2">{beforeAfter.map((item) => <Card key={item.title} className="overflow-hidden p-0"><div className="grid grid-cols-2"><div className="relative"><img src={item.b} className="h-52 w-full object-cover brightness-75" alt={item.before} /><span className="absolute left-2 top-2 rounded-full bg-red-600 px-3 py-1 text-xs font-black text-white">Before</span></div><div className="relative"><img src={item.a} className="h-52 w-full object-cover" alt={item.after} /><span className="absolute left-2 top-2 rounded-full bg-emerald-600 px-3 py-1 text-xs font-black text-white">After</span></div></div><div className="p-4"><h3 className="font-black">{item.title}</h3><p className="mt-1 text-xs font-bold text-slate-500">{item.before} → {item.after}</p></div></Card>)}</div></section><AboutSection /><section id="reviews" className="py-10"><h2 className="mb-6 text-3xl font-black">People Reviews</h2><div className="grid gap-5 md:grid-cols-3">{reviews.map((item) => <Card key={item.name}><h3 className="font-black">{item.name}</h3><p className="text-xs font-bold text-slate-500">{item.area}</p><p className="mt-3 text-yellow-400">★★★★★</p><p className="mt-3 font-bold text-slate-600">“{item.text}”</p></Card>)}</div></section><section id="faq" className="py-10"><h2 className="mb-6 text-3xl font-black">FAQ</h2><div className="grid gap-4 md:grid-cols-2">{[["How is deep cleaning price calculated?", "Furnished is ₹8.5/sq.ft and unfurnished is ₹7.5/sq.ft."], ["Can I book multiple services?", "Yes, add multiple services in one booking."], ["How do I share location?", "Use current location, type address, or paste Google Maps URL."], ["Will WhatsApp confirmation open?", "Yes, after Firestore save, booking details open in WhatsApp to FreshNest number."]].map(([q, a]) => <Card key={q}><h3 className="font-black"><HelpCircle className="mr-1 inline text-yellow-600" /> {q}</h3><p className="mt-2 text-sm font-semibold text-slate-500">{a}</p></Card>)}</div></section></>;
}
...
function FloatingSupport() {
  return <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-3"><button type="button" onClick={() => alert("Call FreshNest: 86374 14008")} className="rounded-full bg-[#111827] p-4 text-white shadow-xl"><Phone /></button><button type="button" onClick={() => openSafeUrl(`https://wa.me/${COMPANY_PHONE}?text=${encodeURIComponent("Hi FreshNest, I need help booking a service")}`)} className="rounded-full bg-emerald-600 p-4 text-white shadow-xl"><MessageCircle /></button></div>;
}

function MyBookingPage({ bookings, setPage, repeatLast, sendWhatsApp }) {
  return <div className="min-h-screen bg-[#faf9f5]"><header className="sticky top-0 z-40 border-b bg-white/90 backdrop-blur-xl"><div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4"><button type="button" onClick={() => setPage("home")} className="rounded-xl bg-slate-100 px-4 py-3 font-black"><ArrowLeft className="mr-1 inline" size={16} /> Home</button><h2 className="font-black">My Booking</h2></div></header><main className="mx-auto max-w-5xl px-5 py-8">{bookings.length === 0 ? <Card><p className="text-center font-bold text-slate-500">No bookings yet.</p></Card> : <div className="grid gap-4">{bookings.map((bookingItem) => <Card key={bookingItem.id}><div className="flex justify-between gap-4"><div><p className="text-xs font-black text-slate-400">{bookingItem.id}</p><h3 className="text-xl font-black">{bookingItem.status}</h3><p className="text-sm font-bold text-slate-500">{bookingItem.date} • {bookingItem.slot}</p></div><b className="text-2xl">{money(bookingItem.total)}</b></div><div className="mt-4 grid gap-2">{bookingItem.items.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-3 text-sm font-bold">{item.name} • {item.qty} {item.unit} • {money(item.total || item.price * item.qty)}</div>)}</div><div className="mt-4 grid gap-2 md:grid-cols-5">{bookingItem.timeline.map((step) => <div key={step} className="rounded-xl bg-emerald-50 p-2 text-xs font-black text-emerald-700">{step}</div>)}</div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={repeatLast} className="rounded-xl bg-yellow-400 px-4 py-3 font-black">Repeat Booking</button><button type="button" onClick={() => sendWhatsApp(bookingItem)} className="rounded-xl bg-emerald-600 px-4 py-3 font-black text-white">WhatsApp Invoice</button></div></Card>)}</div>}</main></div>;
}
